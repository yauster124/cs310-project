...
def plot_curves(model, A=None, B=None, C=None, D=None):
    if not A:
        A = np.load(f'output/{model}_epoch_training_losses.npy')
        B = np.load(f'output/{model}_epoch_training_scores.npy')
        C = np.load(f'output/{model}_epoch_test_loss.npy')
        D = np.load(f'output/{model}_epoch_test_score.npy')
    ...
    plt.ylabel('accuracy')
    plt.legend(['train', 'test'], loc="upper left")
    title = f"output/{model}_curves.png"
