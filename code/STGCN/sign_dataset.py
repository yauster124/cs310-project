...
def read_pose_file(filepath):
    ...
    save_to = os.path.join('../../../../../../large/u2008310/data/features', vid)
    ...
    try:
        ...
        xy = torch.stack([x, y, conf]).transpose_(0, 1)
        ...
        if not os.path.exists(save_to):
            os.mkdir(save_to)
        torch.save(xy, os.path.join(save_to, frame_id + '_ft.pt'))

        return xy

def read_pose_file2(filepath):
    ...
    save_to = os.path.join('../TGCN/features', vid)
...
class Sign_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # x1 = self._load_poses(video_id, frame_start, frame_end, self.sample_strategy, self.num_samples)
        # x2 = self._load_poses2(video_id, frame_start, frame_end, self.sample_strategy, self.num_samples)
    ...
    def _load_poses_in_order(self, video_id, frame_start, frame_end, sample_strategy, num_samples):
        ...
        if len(poses) < num_samples:
            ...
            pad = [last_pose for i in range(num_padding)]
        ...
        poses = torch.flatten(poses, start_dim=1)
        print(poses[0][89])
    ...
    def _load_poses2(self, video_id, frame_start, frame_end, sample_strategy, num_samples):
            """ Load frames of a video. Start and end indices are provided just to avoid listing and sorting the directory unnecessarily.
            """
            poses = []
            ...
            for i in frames_to_sample:
                pose_path = os.path.join(self.pose_root, video_id, self.framename.format(str(i).zfill(5)))
                pose = read_pose_file2(pose_path)
...
if __name__ == '__main__':
    ...
    for data in train_data_loader:
        X, y, _ = data
        # # ONLY TOP METHOD WORKS
        # X1 = torch.stack(torch.chunk(X, 50, 2))
        # X1 = torch.permute(X1, (1, 0, 2, 3))
        # X1 = torch.flatten(X1, start_dim=2)
        # X1 = torch.unsqueeze(X1, dim=3)
        # X1 = X1.permute(0, 2, 1, 3)
        # N, _, T, M = X1.shape
        # X1 = X1.reshape(N, 3, T, 53, M)

        # X2 = torch.unsqueeze(X, dim=3)
        # X2 = X2.permute(0, 2, 1, 3)
        # N, _, V, M = X2.shape
        # X2 = X2.reshape(N, 3, 50, V, M)
        # print(X[0][2])
        # print(X[0][2][50])
        # print(X1[0][1][0][2][0])
        # print(X2[0][1][0][2][0])
